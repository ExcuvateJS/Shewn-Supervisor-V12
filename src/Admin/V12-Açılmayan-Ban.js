...
exports.run = async (client, message, args) => {
    ...
    const ban = async (user, reason) => {
        if(!message.guild.me.permissions.has("BAN_MEMBERS")) return message.channel.send('Bu komutu kullanabilmek için gerekli role sahip değilim.');

        try {
            const executor = await message.author;
            const bans = databaseNotBan.get(`acilmayanBan.shewn.${message.guild.id}`) || [];
            ...
            const banID = databaseNotBan.add(`acilmayanBanID_${message.guild.id}`, 1);
            ...
            databaseNotBan.push(`acilmayanBan.shewn.${message.guild.id}`, { id: banID, executor, user, reason, time: new Date() });
        } catch (err) {
            if (err.code == 50035) {
    ...
    const unban = async (user) => {
        ...
        try {
            ...
            databaseNotBan.set(`acilmayanBan.shewn.${message.guild.id}`, bans);
        } catch (err) {
            if (err.code == 50035) {
                error(`\`${err.message.split('"')[1]}\` geçerli bir üye veya ID değil.`);
            } else {
...
exports.conf = {
    enabled: true,
    guildOnly: false,
    aliases: ['açılmayanban', 'aban', 'notban'],
    permLevel: 0
};

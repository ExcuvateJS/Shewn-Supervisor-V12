...
exports.run = async(client, message, args) => {
    ...
    if (!user) return message.channel.send(embed.setDescription(`${message.author} kime ceza vereceğini yazmadın! \`!vmute @Shewn/ID <sebep> <süre>\``).setTimestamp().setFooter(ayar.footer)).then(m => m.delete({ timeout: 7000 }) && message.delete({ timeout: 7000 }))
    if (!user.voice.channel) return message.react(Config.Emoji.Çarpı)
    if (!args[1]) return message.channel.send(embed.setDescription(`${message.author} ceza süresini yazmadın! \`!vmute @Shewn/ID <sebep> <süre\``).setTimestamp().setFooter(ayar.footer)).then(m => m.delete({ timeout: 7000 }) && message.delete({ timeout: 7000 }))
    let sure = args[1]
        .replace("s", " Saniye")
        .replace("m", " Dakika")
        .replace("h", " Saat")
    ...
    if (durum) return message.channel.send(embed.setDescription(`Kullanıcı zaten muteli.`).setTimestamp().setFooter(Config.Bots.footer)).then(x => x.delete({ timeout: 7000 }) && message.delete({ timeout: 7000 }))

    user.voice.setMute(true)
    message.react(Config.Emoji.Onay)
    message.channel.send(embed.setDescription(`${user} kullanıcısı ${message.author} tarafından **${reason}** sebebiyle \`${sure}\` süresince voice-mute cezası aldı.`).setTimestamp().setFooter(Config.Bots.footer)).then(x => x.delete({ timeout: 7000 }) && message.delete({ timeout: 7000 }))
    ...
    client.channels.cache.get(Config.Logs.VmuteLogID).send(embed.setDescription(`
 ...
 `).setTimestamp().setFooter(Config.Bots.footer))

    setTimeout(async() => { await kdb.delete(`durum.${user.id}.vmute`)
        if (user.voice.channel) user.voice.setMute(false)
        let log = message.guild.channels.cache.get(Config.Logs.VmuteLogID)
        if (log) log.send(embed.setDescription(`${user} kullanıcısının **voice-mute** ceza süresi bittiği için mutesini açtım.`).setTimestamp().setFooter(Config.Bots.footer))}, ms(args[1]))}

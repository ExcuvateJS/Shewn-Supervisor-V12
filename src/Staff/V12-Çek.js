...
exports.run = async (client, message, args) => {
    ...
    let ExcuvateJSON = new Discord.MessageEmbed()
        .setColor("BLUE")
        .setDescription(`${Kullanıcı},, ${message.author} **Adlı Kullanıcı Sizi Odasına Çekmek İstiyor Kabul Ediyormusunuz** `)
    ...
    ExcuvateReaction.awaitReactions(filter, {
        ...
        if (reaction.emoji.id === Config.Emoji.Onay) {
            let shewn = new Discord.MessageEmbed()
                .setColor("GREEN")
                .setDescription(`${Kullanıcı} Başarılı Bir Şekilde Ses Odasına Çekildi`)
            message.channel.send(shewn)
            Kullanıcı.voice.setChannel(message.member.voice.channel)
            ExcuvateReaction.delete()
        } else {
            let ExcuvateRedd = new Discord.MessageEmbed()
                .setColor("RED")
                .setDescription(`${Kullanıcı} Odaya Çekmeyi Kabul Etmedi`)
